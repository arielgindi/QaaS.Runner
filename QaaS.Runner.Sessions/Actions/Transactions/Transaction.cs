...
public class Transaction : StagedAction
{
    ...
    public Transaction(
        ...
        DataFilter outputDataFilter,
        Policy? policies,
        bool loop,
        int iterations,
        ulong sleepTimeMs,
        SerializationType? serializationType,
        SerializationType? deserializationType,
        ...
        : base(name, stage, policies, logger)
    {
        ...
        if (_parallelism != null)
            InitializeSemaphore(_parallelism.Value);
    ...
    private void InitializeSemaphore(int connectionAcceptanceValue)
    {
        ...
        Logger.LogDebug(
            "Transaction Parallelism Semaphore initiated with max parallelism of {MaxConnections}",
            maxConnections
        );
    }
    ...
    {
        _generatedData = EnumerableExtensions
        ...
        _iterableSerializableSaveIterator = new IterableSerializableDataIterator(
            _generatedData,
            _serializer
        );
        Logger.LogDebug(
    ...
    internal override InternalCommunicationData<object> Act()
    {
        ...
        try
        {
            ...
            {
                Logger.LogDebug(
                    "Starting transaction {ActionName} iteration {Iteration}. Mode={Mode}",
                    Name,
                    iteration + 1,
                ...
                Logger.LogDebug(
                    "Finished transaction {ActionName} iteration {Iteration}. Sleeping {SleepTimeMs} ms",
                    Name,
                    iteration,
                    _sleepTimeMs
                );
        ...
        Logger.LogDebug(
            "Finished transaction {ActionName}. Inputs={InputCount}, Outputs={OutputCount}",
            Name,
            data.Input?.Count ?? 0,
            data.Output?.Count ?? 0
        );
    ...
    private bool Transact(InternalCommunicationData<object> actData)
    {
        ...
        try
        {
            _iterableSerializableSaveIterator.ApplyToAll(
                dataToTransact,
                dataPair =>
                {
                    var currentIndex = Interlocked.Increment(ref pairIndex) - 1;
                    ...
                    LogData(
                        actData,
                        dataPair
                            .Original.CloneDetailed(transactionData.Item1.Timestamp)
                            .AddIoMatchIndexToDetailedData(currentIndex),
                        InputOutputState.OnlyInput
                    );
                    var response = transactionData.Item2?.AddIoMatchIndexToDetailedData(
                        currentIndex
                    );
                    if (response != null)
                    {
                        LogData(
                            actData,
                            response
                                .CloneDetailed(transactionData.Item2?.Timestamp)
                                .AddIoMatchIndexToDetailedData(currentIndex),
    ...
    )
    {
        ...
        runningSession.Outputs!.Add(_receivedRunningCommunicationData);
    }
    ...
    )
    {
        itemBeforeSerialization = (
            _deserializer != null
                ? GetDeserializedData(itemBeforeSerialization)
                : itemBeforeSerialization
